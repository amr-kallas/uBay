...
const API = {
  getAll: async ({ id, ...params }: ParamsWithId) => {
    const { data } = await axios.get<API_List<Comment>>(
      API_ROUTES.Post.COMMENTS(id),
      {
  ...
  editComment: async ({ id, body }: EditComment) => {
    const { data } = await axios.patch(API_ROUTES.Comment.EDIT(id), body)
    return data
  },
